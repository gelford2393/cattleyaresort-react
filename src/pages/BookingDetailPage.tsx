...
interface PaymentRecord { id: string; type: string; date: string; referenceNo: string; amount: number; }
...
export function BookingDetailPage() {
  ...
  const handleAddDiscount = async (discount: { careOfBy: string; others: string; amount: number }) => {
    ...
    const newTotal = booking.subTotal - newDiscounts.reduce((s, d) => s + d.amount, 0) +
  ...
  const handleAddAdditional = async (item: { description: string; amount: number }) => {
    ...
    const newTotal = booking.subTotal - (booking.discount ?? 0) +
