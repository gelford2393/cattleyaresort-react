...
function SelectedSlotsPanel({ control }: { control: Control<ReserveInput> }) {
  const slots = useWatch({ control, name: "slots" });
  const subTotal = slots.reduce((sum, s) => sum + s.rate, 0);
...
export function ReserveForm({
      ...
      ? [
          {
            ...
            rate:
              defaultSlotType === "DAY"
                ? preSelectedPool.dayRate
                : preSelectedPool.nightRate,
  ...
  useEffect(() => {
    ...
    const filtered = current.filter(
      (s) => !takenSlots.some((t) => t.pool === s.pool && t.type === s.type),
    );
  ...
  const toggleSlot = (
    ...
    const current = form.getValues("slots");
    const exists = current.find((s) => s.pool === pool && s.type === type);
    const next: SlotInput[] = exists
      ? current.filter((s) => !(s.pool === pool && s.type === type))
      : [...current, { pool, type, rate, depositRate }];
    form.setValue("slots", next, { shouldValidate: true });
  };

  const isSelected = (pool: string, type: "DAY" | "NIGHT") =>
    form.getValues("slots").some((s) => s.pool === pool && s.type === type);

  const isTaken = (pool: string, type: "DAY" | "NIGHT") =>
    takenSlots.some((s) => s.pool === pool && s.type === type);

  const onSubmit = form.handleSubmit((values) => {
    const subTotal = values.slots.reduce((sum, s) => sum + s.rate, 0);
    ...
    <form onSubmit={onSubmit}>
      ...
      <div
        className={
          hidePoolSelector
            ? undefined
            : "grid lg:grid-cols-[1fr_400px] gap-[var(--s1)] items-start"
        }
      >
        {!hidePoolSelector && (
          <Box>
              ...
              <Box className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-[var(--s-1)]">
                {pools.map((pool) => {
                  ...
                  return (
                    ...
                    >
                      <p className="text-base font-bold tracking-wide mb-3 leading-none">
                        {pool.pool}
                      </p>
                      <Stack gap="s-2">
                        {/* Day slot */}
        ...
        <div className="space-y-[var(--s0)]">
          <SelectedSlotsPanel control={form.control} />
          ...
          <Card>
            ...
            <CardContent className="space-y-[var(--s0)]">
              ...
              <Stack gap="s-2">
                ...
                <DatePicker
                  value={form.getValues("bookingDate")}
                  onChange={(date) => {
          ...
          <Button
            type="submit"
            disabled={createBooking.isPending || form.getValues("slots").length === 0}
            className="w-full"
          >
